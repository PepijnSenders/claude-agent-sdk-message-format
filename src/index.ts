...
function formatToolParamValue(value: unknown): string {
  ...
  if (Array.isArray(value)) {
    ...
    return `[${value.slice(0, 3).map(v => formatToolParamValue(v)).join(', ')}, ... +${value.length - 3} more]`;
  }
...
function validateResultMessage(message: Extract<SDKMessage, { type: 'result' }>): void {
  const requiredFields = ['duration_ms', 'total_cost_usd', 'usage', 'num_turns', 'session_id', 'uuid'];
...
function formatResultMessage(message: Extract<SDKMessage, { type: 'result' }>): string {
  ...
  if (message.usage.cache_read_input_tokens) {
    lines.push(`  ${pc.dim('Cache Read:')} ${pc.cyan(message.usage.cache_read_input_tokens.toLocaleString())}`);
  }
  if (message.usage.cache_creation_input_tokens) {
    lines.push(`  ${pc.dim('Cache Creation:')} ${message.usage.cache_creation_input_tokens.toLocaleString()}`);
  }
  ...
  if (Object.keys(message.modelUsage).length > 0) {
    ...
    for (const [model, usage] of Object.entries(message.modelUsage)) {
      ...
      if (usage.cacheReadInputTokens) {
        lines.push(`    ${pc.dim('Cache Read:')} ${pc.cyan(usage.cacheReadInputTokens.toLocaleString())}`);
      }
      if (usage.cacheCreationInputTokens) {
        lines.push(`    ${pc.dim('Cache Creation:')} ${usage.cacheCreationInputTokens.toLocaleString()}`);
      }
  ...
  if (message.subtype === 'init') {
    ...
    if (message.mcp_servers.length > 0) {
      ...
      for (const server of message.mcp_servers) {
        const statusEmoji = server.status === 'connected' ? pc.green('✓') :
                          server.status === 'failed' ? pc.red('✗') :
                          server.status === 'needs-auth' ? pc.yellow('⚠') : pc.dim('○');
    ...
    lines.push(`   ${pc.dim('Previous tokens:')} ${message.compact_metadata.pre_tokens.toLocaleString()}`);
    return lines.join('\n');
    ...
    if (message.stdout) {
      lines.push(`\n${pc.dim('stdout:')}`);
      lines.push(message.stdout.split('\n').map(l => `  ${l}`).join('\n'));
    }

    if (message.stderr) {
      lines.push(`\n${pc.dim('stderr:')}`);
      lines.push(pc.red(message.stderr.split('\n').map(l => `  ${l}`).join('\n')));
    }
